  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="font-bold" style={{ fontSize: '18px', color: '#e8edf3' }}>Weekly Review</h1>
        <div className="flex items-center gap-2">
          <button onClick={() => setWeekOffset(o => o - 1)} className="p-1.5 rounded-lg" style={{ backgroundColor: '#0d1f35', color: '#7a8a9e' }}>
            <ChevronLeft size={14} />
          </button>
          <span style={{ fontSize: '11px', color: '#7a8a9e', minWidth: '100px', textAlign: 'center' }}>{weekLabel}</span>
          <button onClick={() => setWeekOffset(o => Math.min(o + 1, 0))} disabled={weekOffset === 0} className="p-1.5 rounded-lg disabled:opacity-30" style={{ backgroundColor: '#0d1f35', color: '#7a8a9e' }}>
      ...
      <div className="rounded-xl p-4" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
        <p className="font-bold uppercase tracking-widest mb-3" style={{ fontSize: '10px', color: '#7a8a9e', letterSpacing: '0.06em' }}>Habit Completion</p>
        <ResponsiveContainer width="100%" height={120}>
          <BarChart data={weekData} barCategoryGap="30%">
            <XAxis dataKey="day" tick={{ fill: '#5a6a7e', fontSize: 10, fontFamily: 'Manrope' }} axisLine={false} tickLine={false} />
      ...
      <div className="rounded-xl p-4 flex flex-col gap-3" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
        <p className="font-bold uppercase tracking-widest" style={{ fontSize: '10px', color: '#7a8a9e', letterSpacing: '0.06em' }}>TapWork Hours</p>
        <div className="flex items-center gap-3">
          ...
          />
          <span style={{ fontSize: '12px', color: '#7a8a9e' }}>hours this week</span>
        </div>
      </div>

      {/* Top win & next focus */}
      <div className="rounded-xl p-4 flex flex-col gap-4" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
        <p className="font-bold uppercase tracking-widest" style={{ fontSize: '10px', color: '#7a8a9e', letterSpacing: '0.06em' }}>Review</p>
        {(['top_win', 'next_focus'] as const).map(key => (
          <div key={key} className="flex flex-col gap-2">
            <label style={{ fontSize: '12px', fontWeight: 500, color: '#e8edf3' }}>
            ...
            <textarea
              ...
              style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40', fontSize: '12px' }}
            />
          </div>
        ))}
      </div>
      ...
      {/* 4-week trend */}
      <div className="rounded-xl p-4" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
        <p className="font-bold uppercase tracking-widest mb-3" style={{ fontSize: '10px', color: '#7a8a9e', letterSpacing: '0.06em' }}>4-Week Momentum</p>
        <ResponsiveContainer width="100%" height={100}>
          <LineChart data={trend}>
            <XAxis dataKey="week" tick={{ fill: '#5a6a7e', fontSize: 10, fontFamily: 'Manrope' }} axisLine={false} tickLine={false} />
            <YAxis hide domain={[0, 100]} />
            <Tooltip contentStyle={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40', borderRadius: 8, fontSize: 11 }} itemStyle={{ color: '#fff' }} formatter={(v: number) => [`${v}%`, '']} />
