...
export default function Settings() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="font-bold" style={{ fontSize: '18px', color: '#e8edf3' }}>Settings</h1>

      {/* Profile card */}
      <div className="rounded-xl p-4 flex items-center gap-3" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
        <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold" style={{ backgroundColor: '#1D9E75', color: '#ffffff', fontSize: '16px' }}>
          E
        </div>
        <div>
          <p className="font-bold" style={{ fontSize: '14px' }}>Eshgeen Jafarov</p>
          <p style={{ fontSize: '11px', color: '#7a8a9e' }}>Senior People Analyst · Founder</p>
          <p style={{ fontSize: '10px', color: '#7a8a9e', marginTop: '2px' }}>{user?.email}</p>
        </div>
      </div>

      {/* Manage section */}
      <div>
        <p className="font-bold uppercase tracking-widest mb-2 px-1" style={{ fontSize: '10px', color: '#7a8a9e', letterSpacing: '0.06em' }}>Manage</p>
          ...
          {[
            ...
            >
              <div className="flex-1">
                ...
                <p style={{ fontSize: '10px', color: '#7a8a9e', marginTop: '1px' }}>{desc}</p>
              </div>
              <ChevronRight size={14} style={{ color: '#7a8a9e' }} />
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Data section */}
      <div>
        <p className="font-bold uppercase tracking-widest mb-2 px-1" style={{ fontSize: '10px', color: '#7a8a9e', letterSpacing: '0.06em' }}>Data</p>
        <div className="rounded-xl overflow-hidden" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
          <button
            onClick={exportData}
            className="w-full flex items-center gap-3 px-4 py-3 text-left hover:brightness-110 transition-all"
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p style={{ fontSize: '10px', color: '#7a8a9e', marginTop: '1px' }}>Download as JSON</p>
            </div>
          </button>
          <button
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p style={{ fontSize: '10px', color: '#7a8a9e', marginTop: '1px' }}>Clears all check-in history</p>
            </div>
          </button>
        </div>
      </div>
      ...
      <button
        ...
        style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40', color: '#7a8a9e', fontSize: '13px' }}
      >
        Sign Out
      </button>
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 flex items-center justify-center z-[60] px-4" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }}>
          <div className="w-full max-w-sm rounded-xl p-6 flex flex-col gap-4 text-center" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
            <p className="font-bold" style={{ fontSize: '16px', color: '#e8edf3' }}>Reset all streaks?</p>
            <p style={{ fontSize: '12px', color: '#7a8a9e' }}>This will delete all historical check-in data. This cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowResetConfirm(false)} className="flex-1 py-2.5 rounded-lg font-medium" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40', color: '#7a8a9e', fontSize: '13px' }}>
                Cancel
              </button>
