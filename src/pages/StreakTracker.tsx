...
export default function StreakTracker() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="font-bold" style={{ fontSize: '18px', color: '#e8edf3' }}>Streak Tracker</h1>

      {/* 30-day heatmap */}
      <div className="rounded-xl p-4" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
        <p className="font-bold uppercase tracking-widest mb-3" style={{ fontSize: '10px', color: '#7a8a9e', letterSpacing: '0.06em' }}>30-Day Heatmap</p>
        ...
        <div className="flex items-center gap-3 mt-3 flex-wrap">
          {[['0%', '#1a2a40'], ['1–40%', '#F4F6FA22'], ['41–70%', '#1D9E7544'], ['71–89%', '#1D9E75'], ['90–100%', '#EF9F27']].map(([label, color]) => (
            <div key={label} className="flex items-center gap-1">
              <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: color }} />
              <span style={{ fontSize: '9px', color: '#7a8a9e' }}>{label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Weekly bar chart */}
      <div className="rounded-xl p-4" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
        <p className="font-bold uppercase tracking-widest mb-3" style={{ fontSize: '10px', color: '#7a8a9e', letterSpacing: '0.06em' }}>This Week</p>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="rounded-xl p-3 text-center" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
            <p className="font-bold" style={{ fontSize: '20px', color: '#e8edf3' }}>{value}</p>
            <p style={{ fontSize: '9px', color: '#7a8a9e', marginTop: '2px' }}>{label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-3">
        {streaks.map(({ habit, currentStreak, bestStreak, last14 }) => (
          <div key={habit.id} className="rounded-xl p-4" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <span style={{ fontSize: '16px', color: '#e8edf3' }}>{habit.icon}</span>
                <span style={{ fontSize: '12px', fontWeight: 500, color: '#e8edf3' }}>{habit.name}</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-center">
                  <p className="font-bold" style={{ fontSize: '14px', color: '#1D9E75' }}>{currentStreak}</p>
                  <p style={{ fontSize: '9px', color: '#7a8a9e' }}>current</p>
                </div>
                <div className="text-center">
                  <p className="font-bold" style={{ fontSize: '14px', color: '#EF9F27' }}>{bestStreak}</p>
                  <p style={{ fontSize: '9px', color: '#7a8a9e' }}>best</p>
                </div>
              </div>
            </div>
            <div className="flex gap-1">
              {last14.map((done, i) => (
                <div
                  ...
                  style={{ height: '8px', backgroundColor: done ? '#1D9E75' : '#1a2a40' }}
                />
              ))}
            </div>
          </div>
