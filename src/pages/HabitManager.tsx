  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/settings')} className="p-1.5 rounded-lg" style={{ backgroundColor: '#0d1f35', color: '#6B7280' }}>
          <ArrowLeft size={14} />
        </button>
          ...
          {(provided) => (
            ...
            >
              <div className="px-4 py-2.5" style={{ borderBottom: '1px solid #1a2a40' }}>
                <p className="font-bold uppercase tracking-widest" style={{ fontSize: '10px', color: '#6B7280', letterSpacing: '0.06em' }}>Active ({activeHabits.length})</p>
              </div>
              {activeHabits.map((habit, index) => (
                <Draggable key={habit.id} draggableId={habit.id} index={index}>
                  {(prov, snap) => (
                    ...
                    >
                      <div {...prov.dragHandleProps} style={{ color: '#6B7280', cursor: 'grab' }}>
                      ...
                      <button
                        ...
                        style={{ backgroundColor: '#0A1628', color: '#6B7280' }}
                      >
                        <Edit2 size={12} />
                      </button>
                      <button
      ...
      {pausedHabits.length > 0 && (
        <div className="rounded-xl overflow-hidden" style={{ backgroundColor: '#0d1f35', border: '1px solid #1a2a40' }}>
          <div className="px-4 py-2.5" style={{ borderBottom: '1px solid #1a2a40' }}>
            <p className="font-bold uppercase tracking-widest" style={{ fontSize: '10px', color: '#6B7280', letterSpacing: '0.06em' }}>Paused ({pausedHabits.length})</p>
          </div>
      ...
      <button
        ...
        style={{ border: '2px dashed #1a2a40', color: '#6B7280', fontSize: '13px', backgroundColor: 'transparent' }}
      >
